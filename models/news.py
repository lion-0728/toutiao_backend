...
class Category(Base):
    ...
    name: Mapped[str] = mapped_column(String(50), unique=False, comment="分类名称")
...
class News(Base):
    ...
    # 创建索引：提升查询速度 -> 添加目录
    __table_args__ = (
        Index('fk_news_category_idx', 'category_id'),  # 高频查询场景, 索引名称自定义, 索引字段为分类ID
        Index('idx_publish_time', 'publish_time')      # 按发布时间排序, 索引名称自定义, 索引字段为发布时间
    )
